...
function renderPlanner(){
  ...
  const detailRow=({client,plan,planIndex},child=false)=>{
    ...
    return {html:`<tr class="${child?'plan-client-child':''}"><td><b>${esc(client.name)}</b><div class="muted">${esc(canonicalBranch(client,plan.branch)||'GENERAL')} · ${esc(plan.matrixName||plan.groupId||'TODAS LAS MATRICES')} · ${esc(plan.frequency)} · ${esc(plan.service||'MONITOREO')}</div></td>${cells}</tr>`,periodStates};
  };
  ...
  for(const group of groups){
    ...
    const branchCount=new Set(plans.map(x=>norm(x.canonicalBranch(client,plan.branch)||'GENERAL'))).size;
