...
// A7.0.35: identidad canónica de sucursal compartida con el catálogo.
// Una corrección/unificación debe reflejarse también en planes históricos y en la matriz anual.
function canonicalBranch(client,value){
  let current=String(value??'').trim().toUpperCase().replace(/\s+/g,' ');
  ...
  for(const h of hist){
    const aliases=(h.aliases||[]).map(norm);
    if(aliases.includes(norm(current))) current=String(h.official||current).trim().toUpperCase().replace(/\s+/g,' ');
  }
  return current;
}
function canonicalizeClientPlans(client){
  const plans=Array.isArray(client?.monitoringPlans)?client.monitoringPlans:[];
  let changed=false;
  const mapped=plans.map(p=>{const b=canonicalBranch(client,p.branch);if(norm(b)!==norm(p.branch)){changed=true;return {...p,branch:b}}return p});
  return {plans:mapped,changed};
}
...
function samplePlanIdentityMatch(sample,client,plan){
  ...
  if(plan.branch&&norm(canonicalBranch(client,sample.branch))!==norm(canonicalBranch(client,plan.branch)))return false;
...
async function reload(){
  clientCache=await sampleRegistryService.clients();sampleCache=await sampleRegistryService.all();matrixCache=await sampleRegistryService.matrices();
  // Reparación anti-regresión: si un plan antiguo conserva un alias ya unificado,
  // se corrige una sola vez mediante el repositorio normal. No añade polling ni toca Live Sync.
  for(const client of clientCache){
    const fixed=canonicalizeClientPlans(client);
    if(!fixed.changed)continue;
    client.monitoringPlans=fixed.plans;
    try{await repositories.clients.update(client.id,{monitoringPlans:fixed.plans},{userId:'QUALITY_BRANCH_CANONICAL_REPAIR'});}catch(e){console.warn('PEP canonical plan repair:',e);}
  }
}
...
function branchOptions(){const c=selectedClient();const fromClient=(c?.branches||[]).map(b=>canonicalBranch(c,b));const visible=[...document.querySelectorAll('.catalogBranchInput')].map(x=>canonicalBranch(c,x.value)).filter(Boolean);return [...new Set([...fromClient,...visible])];}
...
function inlinePlanDraft(){const c=selectedClient();return {id:uid(),branch:canonicalBranch(c,$('paBranch')?.value||''),...matrixFromSelect($('paMatrix')),service:($('paService')?.value||'MONITOREO').trim()||'MONITOREO',frequency:$('paFrequency')?.value||'MENSUAL',dueDay:Number($('paDueDay')?.value||30),active:true};}
...
function openPlanningDate(clientId,planId,year,label){
  ...
  $('planningDateSubtitle').textContent=`${client.name} · ${canonicalBranch(client,plan.branch)||'GENERAL'} · ${plan.matrixName||plan.groupId||'TODAS'} · ${pp.label}`;
...
function planningAlerts(year){
  ...
  for(const client of clientCache)for(const [planIndex,plan] of (client.monitoringPlans||[]).entries()){if(plan.active===false)continue;for(const pp of periods(plan,year,client)){
    ...
    if(scheduled){const d=dayDiff(today,scheduled);if(d===1)alerts.push({...base,level:'warning',kind:'warning',title:'Monitoreo programado para mañana',detail:`Fecha programada ${humanDate(scheduled)}.`});else if(d===0)alerts.push({...base,level:'warning',kind:'warning',title:'Monitoreo programado para hoy',detail:`Debe ejecutarse hoy ${humanDate(scheduled)}.`});else if(d<0)alerts.push({...base,level:'critical',kind:'warning',title:'Monitoreo planificado pendiente',detail:`La fecha programada ${humanDate(scheduled)} ya pasó y no existe muestra cumplida.`});else if(d<=7)alerts.push({...base,level:'info',kind:'upcoming',title:`Próximo monitoreo · en ${d} días`,detail:`Trabajo programado para ${humanDate(scheduled)}.`});
    }else{const dd=dayDiff(today,due);if(dd===1)alerts.push({...base,level:'warning',kind:'warning',title:'Fecha límite mañana · sin planificación',detail:`Límite ${humanDate(due)} y todavía no tiene fecha programada.`});else if(dd<=0)alerts.push({...base,level:'critical',kind:'warning',title:'Monitoreo pendiente / vencido',detail:`Límite ${humanDate(due)} sin muestra cumplida ni fecha programada.`});else if(dd<=7)alerts.push({...base,level:'info',kind:'upcoming',title:`Próxima obligación · ${dd} días`,detail:`Límite ${humanDate(due)} · todavía sin fecha programada.`});}
  }}
...
function renderPlanningAlerts(year){
  ensurePlanningBell();const alerts=planningAlerts(year);window.dispatchEvent(new CustomEvent('pep:planning-alerts',{detail:{alerts:alerts.map(a=>({level:a.level,kind:a.kind||'warning',title:a.title,detail:a.detail,clientId:a.client.id,clientName:a.client.name,planId:a.plan.id||`INDEX-${a.planIndex}`,year:a.pp.year,label:a.pp.label,branch:canonicalBranch(a.client,a.plan.branch)||'GENERAL',matrix:a.plan.matrixName||a.plan.groupId||'TODAS'}))}}));if($('planningBellCount'))$('planningBellCount').textContent=alerts.length;const tab=document.querySelector('.tab[data-view="intelligentPlanning"]');if(tab){let badge=tab.querySelector('.planning-tab-alert-count');if(!badge){badge=document.createElement('span');badge.className='count planning-tab-alert-count';tab.appendChild(badge)}badge.textContent=alerts.length;badge.style.display=alerts.length?'inline-flex':'none';badge.title=alerts.length?`${alerts.length} alerta(s) de monitoreo`:'Sin alertas de monitoreo';}const panel=$('planningAlertPanel');if(!panel)return;panel.innerHTML=`<div class="modal-head"><div><h3 style="margin:0">🔔 Alertas inteligentes de monitoreo</h3><div class="muted">Se recalculan automáticamente según muestras, fechas límite, planificación y DETENIDAS.</div></div><button class="close" type="button" id="planningAlertsClose">×</button></div>${alerts.length?alerts.map(a=>`<div class="planning-alert-row ${a.level}" data-alert-plan-client="${esc(a.client.id)}" data-alert-plan-id="${esc(a.plan.id||`INDEX-${a.planIndex}`)}" data-alert-plan-year="${a.pp.year}" data-alert-plan-label="${esc(a.pp.label)}"><div class="planning-alert-title">${a.level==='critical'?'🔴':a.level==='info'?'📌':'🟠'} ${esc(a.title)}</div><div><b>${esc(a.client.name)}</b> · ${esc(canonicalBranch(a.client,a.plan.branch)||'GENERAL')} · ${esc(a.plan.matrixName||a.plan.groupId||'TODAS')}</div><div class="planning-alert-meta">${esc(a.pp.label)} · ${esc(a.detail)}</div><button class="btn secondary small" type="button" style="margin-top:6px" data-alert-schedule>📅 Programar / cambiar fecha</button></div>`).join(''):'<div class="empty">✅ No hay alertas de monitoreo en este momento.</div>'}`;$('planningAlertsClose').onclick=()=>panel.classList.remove('show');panel.querySelectorAll('[data-alert-schedule]').forEach(b=>b.onclick=()=>{const r=b.closest('[data-alert-plan-client]');openPlanningDate(r.dataset.alertPlanClient,r.dataset.alertPlanId,r.dataset.alertPlanYear,r.dataset.alertPlanLabel);});
}

function ensurePlannerControls(){
  ensureQuickPlanBanner();if(!$('planningYear'))return;const now=new Date();$('planningYear').value=now.getFullYear();$('planningMonth').value=now.getMonth()+1;ensurePlanningDateModal();ensurePlanningBell();
  if(!$('plannerSmartControls')){
    const host=document.createElement('div');host.id='plannerSmartControls';host.className='planner-smart-controls';host.innerHTML=`<div class="grow"><label>Buscar cliente / sucursal / matriz</label><input id="plannerSearch" type="search" placeholder="Ej. ROSADO, GUAYAQUIL, RESIDUAL"></div><div><label>Estado</label><select id="plannerStatus"><option value="ALL">Todos</option><option value="ACTION">⚠ Requieren acción</option><option value="late">⛔ Vencidos</option><option value="stopped">🔴 Detenidos</option><option value="planned">🟠 Planificados</option><option value="pending">⬜ Pendientes</option><option value="done">✅ Cumplidos</option></select></div><div><label>Frecuencia</label><select id="plannerFrequency"><option value="ALL">Todas</option>${Object.keys(RULES).map(x=>`<option value="${x}">${x}</option>`).join('')}</select></div>`;
    $('planningMatrix').before(host);
    let timer;$('plannerSearch').addEventListener('input',e=>{clearTimeout(timer);timer=setTimeout(()=>{plannerSearch=norm(e.target.value);renderPlanner()},180)});$('plannerStatus').addEventListener('change',e=>{plannerStatus=e.target.value;renderPlanner()});$('plannerFrequency').addEventListener('change',e=>{plannerFrequency=e.target.value;renderPlanner()});
  }
  $('planningYear').addEventListener('change',renderPlanner);$('planningMonth').addEventListener('change',renderPlanner);$('planningRefresh').addEventListener('click',async()=>{await reload();renderPlanner()});
} 
...
function renderPlanner(){
  ...
  const matchesGroup=({client,plans})=>{
    const text=norm([client.name,...plans.flatMap(({plan})=>[canonicalBranch(client,plan.branch),plan.matrixName,plan.groupId,plan.frequency,plan.service])].join(' '));if(plannerSearch&&!text.includes(plannerSearch))return false;
    if(plannerFrequency!=='ALL'&&!plans.some(({plan})=>norm(plan.frequency)===plannerFrequency))return false;
    if(plannerStatus!=='ALL'){
      const types=plans.flatMap(({plan})=>getStates(client,plan).map(x=>x.st.type));
      if(plannerStatus==='ACTION'){if(!types.some(x=>['late','stopped','pending'].includes(x)))return false;}else if(!types.includes(plannerStatus))return false;
    }return true;
  };
  const singles=groups.filter(g=>g.plans.length===1),multis=groups.filter(g=>g.plans.length>1);
  const source=(plannerSection==='SINGLE'?singles:multis).filter(matchesGroup);
  const severity={late:5,stopped:4,planned:3,pending:2,done:1};
  const detailRow=({client,plan,planIndex},child=false)=>{
    const periodStates=getStates(client,plan);const cells=MONTHS.map((m,idx)=>{const entries=periodStates.filter(x=>x.displayMonth===idx+1);if(!entries.length)return '<td class="plan-na">—</td>';const cellType=entries.slice().sort((a,b)=>(severity[b.st.type]||0)-(severity[a.st.type]||0))[0]?.st.type||'pending';const content=entries.map(({pp,st})=>{const detail=st.chain?.samples?.length?chainHtml(st.chain,pp):esc(st.text);const alert=st.type==='stopped'&&!st.scheduledDate?`<span class="retake-alert">⚠ PLANIFICAR ${esc(st.nextLabel)}</span>`:'';const scheduled=st.scheduledDate?`<span class="plan-date">📅 ${esc(humanDate(st.scheduledDate))}${st.type==='stopped'?' · RETOMA':''}</span>`:'';const locked=!!st.chain?.inAnalysis;const clickable=(st.type==='done'||locked)?'':` data-plan-schedule data-client-id="${esc(client.id)}" data-plan-id="${esc(plan.id||`INDEX-${planIndex}`)}" data-plan-year="${pp.year}" data-plan-label="${esc(pp.label)}"`;const lockNote=locked?`<span class="plan-date">🔒 Código registrado · esperando ingreso a Laboratorio</span>`:'';return `<div${clickable} title="${esc(pp.label)} · límite ${esc(pp.due)}">${detail}${alert}${scheduled}${lockNote}<div class="muted">${esc(pp.label)}</div></div>`}).join('<hr style="border:0;border-top:1px solid #dce6df;margin:5px 0">');return `<td class="plan-cell-${cellType}${entries.some(x=>x.st.type!=='done'&&!x.st.chain?.inAnalysis)?' plan-clickable':''}">${content}</td>`}).join('');
    return `<tr class="${child?'plan-client-child':''}"><td><b>${esc(client.name)}</b><div class="muted">${esc(canonicalBranch(client,plan.branch)||'GENERAL')} · ${esc(plan.matrixName||plan.groupId||'TODAS LAS MATRICES')} · ${esc(plan.frequency)} · ${esc(plan.service||'MONITOREO')}</div></td>${cells}</tr>`;
  };
  let body='';
