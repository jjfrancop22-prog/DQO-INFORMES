...
function table(rows,kind){if(!rows.length)return '<div class="empty">No hay registros en esta bandeja.</div>';const billingReadOnly=isBillingMonitoringReadOnly();const showFrequency=kind==='registry'||kind==='analysis',showAnalysisSelect=kind==='analysis'&&analysisBulkMode&&!billingReadOnly;return `<div class="table-wrap"><table><thead><tr>${showAnalysisSelect?'<th style="width:42px"><input type="checkbox" id="analysisSelectAll" title="Seleccionar visibles"></th>':''}<th>Código</th><th>Año</th><th>Cliente / Sucursal</th><th>Matriz / Grupo</th><th>Fecha muestra</th><th>Recepción</th>${showFrequency?'<th>Frecuencia / mes</th>':''}<th>DQO/Tenso</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>${rows.map(x=>{const req=x.workflow?.requirements||{},vals=x.workflow?.analysisValues||{},hasSpecial=!!(req.dqo||req.surfactants);let actions=billingReadOnly?`<button class="btn secondary small" data-history="${x.id}">Historial</button>`:`<button class="btn blue small" data-edit="${x.id}">Editar</button> ${kind==='registry'&&x.workflow?.decisionStatus==='CONTINUAR'?`<button class="btn warning small" data-quality-stop="${x.id}">Corregir a DETENIDA</button> `:''}<button class="btn secondary small" data-history="${x.id}">Historial</button> <button class="btn danger small" data-safe-delete-sample="${x.id}" data-safe-delete-stage="MONITORING">Eliminar</button>`;if(!billingReadOnly&&kind==='analysis')actions=`<button class="btn primary small" data-analysis="${x.id}">${hasSpecial?'Registrar análisis':'Registrar análisis · continuar'}</button> ${actions}`;if(!billingReadOnly&&kind==='waiting')actions=`<button class="btn warning small" data-waiting="${x.id}">Resolver</button> ${actions}`;return `<tr>${showAnalysisSelect?`<td><input type="checkbox" data-analysis-select="${x.id}" ${analysisSelected.has(x.id)?'checked':''}></td>`:''}<td>${kind==='registry'?`<button type="button" data-report-deadline="${x.id}" title="Ver fecha máxima de emisión del informe" style="border:0;background:transparent;padding:0;color:inherit;font:inherit;font-weight:900;cursor:pointer;text-decoration:underline;text-decoration-style:dotted;text-underline-offset:3px">📅 ${esc(x.code)}</button>`:`<b>${esc(x.code)}</b>`}<div class="muted mono">${esc(x.id.slice(0,8))}…</div></td><td>${esc(x.year)}</td><td><b>${esc(x.clientId)}</b><div class="muted">${esc(x.branch||'—')}</div></td><td><b>${esc(x.matrixId)}</b><div class="muted">${esc(x.groupId)}</div></td><td>${formatDate(x.samplingDate)}</td><td>${formatDate(x.receivedDate)}</td>${showFrequency?`<td><b>${esc(x.monthFrequency||'—')}</b></td>`:''}<td>${req.dqo?`DQO: ${esc(vals.dqo||'pendiente')}`:'DQO: —'}<br>${req.surfactants?`Tenso: ${esc(vals.surfactants||'pendiente')}`:'Tenso: —'}</td><td>${statusBadge(x)}</td><td>${actions}</td></tr>`}).join('')}</tbody></table></div>`}
function reportStageLabel(r){if(!r)return 'Pendiente de ingreso a Informes';const s=String(r.reportStatus||'').toUpperCase();if(s==='PENDING_DELIVERY')return 'Informe pendiente de entrega';if(s==='AUTHORIZATION')return 'En autorización';if(s==='PORTAL_PENDING')return 'Pendiente de envío al portal';if(s==='PORTAL_SENT')return 'Enviado al portal';return s||'Informe creado'}
function reportDeadlineBadge(deadline){if(!deadline)return '<span class="badge">SIN FECHA</span>';const today=new Date().toISOString().slice(0,10);const n=businessDaysBetween(today,deadline);if(n===null)return '<span class="badge">—</span>';if(n<0)return `<span class="badge red">VENCIDO · ${Math.abs(n)} día(s) laborable(s)</span>`;if(n===0)return '<span class="badge amber">VENCE HOY</span>';if(n<=2)return `<span class="badge amber">PRÓXIMO · ${n} día(s) laborable(s)</span>`;return `<span class="badge green">EN PLAZO · ${n} día(s) laborable(s)</span>`}
async function openReportDeadline(sampleId){
  try{
    const sample=allRows.find(x=>x.id===sampleId);if(!sample)return;
    const [lab,reports]=await Promise.all([labService.entryBySample(sampleId),reportsService.all()]);
    const report=reports.find(r=>r.sampleId===sampleId&&!r.deleted)||null;
    const reception=report?.receptionDate||lab?.receptionDate||sample.receivedDate||'';
    const serviceType=report?.serviceType||lab?.serviceType||'';
    const sla=serviceType?slaDaysForService(serviceType):'';
    const deadline=report?.maxReportDate||lab?.maxReportDate||(reception&&serviceType?addLabDays(reception,sla):'');
    $('reportDeadlineTitle').textContent=`📅 Informe · código ${sample.code}`;
    $('reportDeadlineSubtitle').textContent=`${sample.clientId}${sample.branch?' · '+sample.branch:''}`;
    if(!lab?.officialEntryAt&&!report){
      $('reportDeadlineBody').innerHTML=`<div class="notice warn"><b>Aún no existe fecha máxima de emisión.</b><br>La muestra todavía no tiene ingreso oficial al Laboratorio. Cuando se confirme ese ingreso, el sistema calculará automáticamente el SLA y esta consulta mostrará la fecha.</div><div class="grid"><div class="w6"><label>Fecha de muestra</label><div class="matrix-info">${formatDate(sample.samplingDate)}</div></div><div class="w6"><label>Recepción actual</label><div class="matrix-info">${formatDate(sample.receivedDate)}</div></div></div>`;
    }else{
      $('reportDeadlineBody').innerHTML=`<div class="grid"><div class="w4"><label>Recepción Laboratorio</label><div class="matrix-info"><b>${formatDate(reception)}</b></div></div><div class="w4"><label>Tipo de servicio</label><div class="matrix-info"><b>${esc(serviceType||'—')}</b>${sla?` · ${sla} días laborables`:''}</div></div><div class="w4"><label>Analista</label><div class="matrix-info">${esc(report?.analyst||lab?.analyst||'—')}</div></div><div class="w6"><label>Fecha máxima de emisión del informe</label><div class="matrix-info" style="font-size:18px"><b>${formatDate(deadline)}</b></div></div><div class="w6"><label>Estado del plazo</label><div class="matrix-info">${reportDeadlineBadge(deadline)}</div></div><div class="w6"><label>Estado del informe</label><div class="matrix-info">${esc(reportStageLabel(report))}</div></div><div class="w6"><label>Entrega real</label><div class="matrix-info">${formatDate(report?.realDeliveryDate||'')}</div></div></div><div class="notice" style="margin-top:12px"><b>Consulta inteligente:</b> la fecha máxima se toma del mismo cálculo SLA del sistema (${sla||'—'} días laborables según servicio), considerando fines de semana y feriados configurados. Esta ventana es solo de lectura.</div>`;
    }
    $('reportDeadlineModal').classList.add('show');
...
function exportRegistryExcel(){
  const rows=registryExcelRows();if(!rows.length){toast('No hay muestras para exportar en el rango de fechas seleccionado.',true);return}
  ...
  const xml=`<?xml version="1.0"?><?mso-application progid="Excel.Sheet"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet" xmlns:x="urn:schemas-microsoft-com:office:excel">${styles}<Worksheet ss:Name="Registro de Muestras"><Table>${columns}<Row ss:Height="27"><Cell ss:MergeAcross="13" ss:StyleID="Title"><Data ss:Type="String">REGISTRO DE MUESTRAS</Data></Cell></Row><Row ss:Height="22"><Cell ss:MergeAcross="13" ss:StyleID="Meta"><Data ss:Type="String">Generado: ${excelCell(cut)} · Registros: ${rows.length} · ${$('filterFrom').value||$('filterTo').value?'Rango de fechas aplicado':'Base completa'} · Orden: Fecha de muestra ↓ + Código ↓</Data></Cell></Row>${head}${body}</Table><WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel"><FreezePanes/><FrozenNoSplit/><SplitHorizontal>3</SplitHorizontal><TopRowBottomPane>3</TopRowBottomPane><ActivePane>2</ActivePane></WorksheetOptions></Worksheet></Workbook>`;
  const blob=new Blob([xml],{type:'application/vnd.ms-excel;charset=utf-8'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`PEP_REGISTRO_MUESTRAS_${new Date().toISOString().slice(0,10)}.xls`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);toast(`${rows.length} muestra(s) exportadas · ${$('filterFrom').value||$('filterTo').value?'rango de fechas':'base completa'}`)
}
...
function renderRegistryTable(){const r=filteredRegistry();const total=r.length;const pages=Math.max(1,Math.ceil(total/REGISTRY_PAGE_SIZE));registryPage=Math.min(Math.max(1,registryPage),pages);const start=(registryPage-1)*REGISTRY_PAGE_SIZE;const shown=r.slice(start,start+REGISTRY_PAGE_SIZE);$('registryTable').innerHTML=table(shown,'registry');$('registryResultCount').textContent=total?`${shown.length} mostrados de ${total}`:'0 resultados';const info=$('registryPageInfo');if(info)info.textContent=`Página ${registryPage} de ${pages}`;const prev=$('registryPrevPage'),next=$('registryNextPage');if(prev)prev.disabled=registryPage<=1;if(next)next.disabled=registryPage>=pages;const exportHint=$('registryExportHint');if(exportHint){const from=$('filterFrom').value,to=$('filterTo').value;exportHint.textContent=from||to?'Excel: exportará todo el rango de fechas seleccionado':'Excel: exportará toda la base';}}
function renderTables(){const aq=$('searchAnalysis').value.trim().toLowerCase(),wq=$('searchWaiting').value.trim().toLowerCase(),sq=$('searchStopped').value.trim().toLowerCase();const analysisRows=queues.analysis.filter(x=>hay(x,aq));$('analysisTable').innerHTML=table(analysisRows,'analysis');$('waitingTable').innerHTML=table(queues.waiting.filter(x=>hay(x,wq)),'waiting');$('stoppedTable').innerHTML=table(queues.stopped.filter(x=>hay(x,sq)),'stopped');renderRegistryTable();renderAnalysisBulkControls(analysisRows)}
...
function clearFilters(){['searchRegistry','filterYear','filterMatrix','filterGroup','filterDecision','filterFrom','filterTo'].forEach(id=>$(id).value='');registryPage=1;renderRegistryTable()}
...
document.addEventListener('click',e=>{const a=e.target.closest('[data-analysis]'),w=e.target.closest('[data-waiting]'),h=e.target.closest('[data-history]'),ed=e.target.closest('[data-edit]'),qs=e.target.closest('[data-quality-stop]'),cc=e.target.closest('[data-client-catalog]'),mc=e.target.closest('[data-matrix-catalog]'),lo=e.target.closest('[data-lab-open]'),lsa=e.target.closest('[data-lab-save-analyst]'),lh=e.target.closest('[data-lab-history]'),rd=e.target.closest('[data-report-deadline]'),sd=e.target.closest('[data-safe-delete-sample]');if(a)openAnalysis(a.dataset.analysis);if(w)openWaiting(w.dataset.waiting);if(h)openHistory(h.dataset.history);if(ed)openEdit(ed.dataset.edit);if(qs)correctRegistryToStopped(qs.dataset.qualityStop);if(cc)editClientCatalog(cc.dataset.clientCatalog);if(mc)editMatrixCatalog(mc.dataset.matrixCatalog);if(lo)openLabEntry(lo.dataset.labOpen);if(lsa)saveLabAnalyst(lsa.dataset.labSaveAnalyst);if(lh)openLabHistory(lh.dataset.labHistory);if(rd)openReportDeadline(rd.dataset.reportDeadline);if(sd)safeDeleteSample(sd.dataset.safeDeleteSample,sd.dataset.safeDeleteStage)});document.addEventListener('change',e=>{const item=e.target.closest?.('[data-analysis-select]');if(item){if(item.checked)analysisSelected.add(item.dataset.analysisSelect);else analysisSelected.delete(item.dataset.analysisSelect);renderAnalysisBulkControls(queues.analysis.filter(x=>hay(x,$('searchAnalysis').value.trim().toLowerCase())));return}if(e.target.id==='analysisSelectAll'){const visible=queues.analysis.filter(x=>hay(x,$('searchAnalysis').value.trim().toLowerCase()));visible.forEach(x=>e.target.checked?analysisSelected.add(x.id):analysisSelected.delete(x.id));renderTables()}});document.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key==='Enter')$('sampleForm').requestSubmit();if(e.altKey&&e.key.toLowerCase()==='n'){e.preventDefault();showView('register');clearForm()}if(e.altKey&&e.key.toLowerCase()==='r'){e.preventDefault();showView('register');repeatLast()}});
